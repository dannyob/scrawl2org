...
@click.option('--kitty', is_flag=True,
              help='Force Kitty image protocol display (even in non-Kitty terminals)')
@click.option('--no-kitty', is_flag=True,  
              help='Disable Kitty image protocol, output raw binary')
@click.option('--width', type=int,
              help='Display width in terminal cells (Kitty only)')
@click.option('--height', type=int,
              help='Display height in terminal cells (Kitty only)')
def main(pdf_file: Path, database: str, force: bool, verbose: bool, extract: str, output: str, 
         kitty: bool, no_kitty: bool, width: int, height: int):
